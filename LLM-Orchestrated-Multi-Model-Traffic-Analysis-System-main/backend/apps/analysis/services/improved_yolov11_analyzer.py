"""
Improved YOLOv11 Traffic Analysis Service - FIXED VERSION
"""
import os
import cv2
import numpy as np
...
class ImprovedYOLOv11Analyzer:
    def __init__(self, model_path: Optional[str] = None, confidence_threshold: float = 0.25):
        ...
        logger.info(f"Improved YOLOv11 analyzer initialized")
    
    def _get_best_model_path(self) -> str:
        """Get the best available YOLOv11 model - centralized to backend/models/"""
    ...
    def _load_model(self):
        ...
        try:
            ...
            logger.info(f"Successfully loaded YOLOv11 model: {self.model_path}")
    ...
    def detect_vehicles(self, image: np.ndarray) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"YOLOv11 detected {len(detections)} vehicles")
            return detections
    ...
    def _filter_detections(self, detections: List[Dict], 
        ...
        logger.info(f"YOLOv11 Filtering: {len(detections)} -> {len(filtered)} vehicles (removed {len(detections) - len(filtered)} false positives)")
    ...
    def analyze_image(self, image_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if (conditions['visibility'] in challenging_conditions or 
                ...
                logger.info(f"YOLOv11 Adaptive threshold: {original_threshold:.2f} -> {self.confidence_threshold:.2f} (conditions: {conditions['visibility']}, {conditions['density_hint']})")
            ...
            logger.info(f"🚗 YOLOv11 vehicle breakdown: Raw={vehicle_counts}, Grouped={grouped_vehicle_counts}")
            ...
            logger.info(f"YOLOv11 analysis complete: {total_vehicles} vehicles detected (conditions: {conditions['visibility']}, {conditions['density_hint']})")
            return results
    ...
    def create_annotated_image(self, image_path: str, output_path: str) -> bool:
        ...
        try:
            ...
            for detection in detections:
                ...
                label = f"v11i-{display_class}: {confidence:.2f}"
            ...
            # Add summary text
            total_vehicles = len(detections)
            summary = f"YOLOv11 Improved: {total_vehicles} vehicles detected"
