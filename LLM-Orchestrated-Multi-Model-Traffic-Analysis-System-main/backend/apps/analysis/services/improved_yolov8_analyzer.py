...
class ImprovedYOLOv8Analyzer:
    def __init__(self, model_path: Optional[str] = None, confidence_threshold: float = 0.25):
        ...
        logger.info(f"Improved YOLOv8 analyzer initialized")
    
    def _get_best_model_path(self) -> str:
        """Get the best available YOLOv8 model - centralized to backend/models/"""
    ...
    def _load_model(self):
        ...
        try:
            ...
            logger.info(f"Successfully loaded YOLOv8 model: {self.model_path}")
    ...
    def _get_adaptive_confidence_threshold(self, conditions: Dict[str, Any], vehicle_type: str) -> float:
        ...
        final_threshold = max(0.05, min(0.40, final_threshold))
        
        return final_threshold
    
    def _filter_detections(self, detections: List[Dict], 
                          image_shape: Tuple[int, int, int]) -> List[Dict]:
        """Apply additional filtering to remove false positives with adaptive thresholds"""
        if not detections:
            return []
        
        # Analyze image conditions for adaptive filtering
        # Create a dummy image array for condition analysis
        height, width = image_shape[:2]
        ...
        for detection in filtered_by_ratio:
            ...
            margin_y_top = height * 0.10  # Reduced from 15% to 10% for better detection
        ...
        # Remove overlapping detections (improved NMS with adaptive IoU)
        filtered = []
        for detection in filtered_by_position:
            ...
            for existing in filtered:
                ...
                # Adaptive IoU thresholds - lower for dense traffic
                same_class = detection['class_name'] == existing['class_name']
                
                # Estimate if this is dense traffic based on detection count
                is_dense = len(filtered_by_position) > 50
                
                if is_dense:
                    # Lower IoU threshold for dense traffic
                    iou_threshold = 0.3 if same_class else 0.2
                else:
        ...
        # Adaptive maximum detection limit
        initial_count = len(detections)
        if initial_count > 300:
            # Very dense traffic - allow more detections
            max_vehicles = min(250, len(filtered))
        elif initial_count > 100:
            # Dense traffic - moderate limit
            max_vehicles = min(150, len(filtered))
        else:
            # Normal traffic - standard limit
            max_vehicles = min(100, len(filtered))
        
        filtered = filtered[:max_vehicles]
        
        logger.info(f"YOLOv8 Filtering: {len(detections)} -> {len(filtered)} vehicles (removed {len(detections) - len(filtered)} false positives)")
    ...
    def analyze_image(self, image_path: str) -> Dict[str, Any]:
        """Analyze single image and return comprehensive results with adaptive filtering"""
        try:
            # Load image
            ...
            if (conditions['visibility'] in challenging_conditions or 
                ...
                logger.info(f"Adaptive threshold: {original_threshold:.2f} -> {self.confidence_threshold:.2f} (conditions: {conditions['visibility']}, {conditions['density_hint']})")
            ...
            logger.info(f"🚗 YOLOv8 vehicle breakdown: Raw={vehicle_counts}, Grouped={grouped_vehicle_counts}")
            ...
            logger.info(f"YOLOv8 analysis complete: {total_vehicles} vehicles detected (conditions: {conditions['visibility']}, {conditions['density_hint']})")
            return results
    ...
    def create_annotated_image(self, image_path: str, output_path: str) -> bool:
        ...
        try:
            ...
            for detection in detections:
                ...
                label = f"v8i-{display_class}: {confidence:.2f}"
            ...
            # Add summary text
            total_vehicles = len(detections)
            summary = f"YOLOv8 Improved: {total_vehicles} vehicles detected"
